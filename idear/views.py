...
def recruit_apply(req):
    ...
    if req.method == 'POST':
        try:
            ...
            Apply = models.Apply.objects.create(user=user, recruit=recruit, Describe=content)
            ApplyID = Apply.Id
            print ApplyID
        ...
        if req.method == 'GET':
            ...
            if sign == "all":
                ...
                for project in pprojects:
                    res = {}
                    procount = models.Praise.objects.filter(project=project.Id).count()
                    res[0] = project
                    res[1] = procount
                    array2.append(res)
                    print procount
                ...
                for p in array2:
                    projects.append(p[0])
                proBest = array2[0][0]
                proBestpraise = array2[0][1]
                print proBestpraise
                return render_to_response('project/projects.html',
                                          {'projectLabels': models.ProjectLabel.objects.all(), "proBestpraise":proBestpraise,"projects": projects,
