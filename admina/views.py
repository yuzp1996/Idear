...
def user_all(req, page=None):
    ...
    if req.method == "GET":
        ...
        return render(req, 'admina/user_all.html', {"users": users, 'pages': range(1, int(pages)+2)})
    else:
...
def project_all(req, page=None):
    if req.method == "GET":
        ...
        if sign == "all":
            ...
            return render(req, 'admina/project_all.html', {"projects": projects, "Pages": range(1, int(Pages+2)),"Labels": labels})
        else:
            projects = []
            ...
            Pages = math.ceil(ProjectLabelObjs.count() / itemPerPage)
            return render(req, 'admina/project_all.html', {"projects": projects, "Pages": range(1, int(Pages+2)),"Labels": labels})
    else:
        resData = {
            "ProjectName": "",
...
@check_login()
@require_http_methods(["POST"])
def project_deleteProject(req):
    resData = {
        "status": 0,
        "message":""
    }
    try:
        deleteId = req.POST['projectId']
        print("Try to delete Project with ID " + deleteId)
        models.Project.objects.get(Id=deleteId).delete()
        resData["message"] = "Success"
        resData["status"] = 1
...
def user_delete(req):
    ...
    try:
        print("Try to delete User with ID " + req.POST['deleteId'])
        models.User.objects.get(Id=req.POST['deleteId']).delete()
        resData["message"] = "Success"
        resData["status"] = 1
    except Exception as e:
        resData["message"] = "服务器异常"
...
def allUserMail(req):
    if req.method == "GET":
        ...
        return render(req, 'admina/allusersmail.html',{
            "users": user
            })
    else:
        try:
            ...
            context = postData.get('context')
            mailName = postData.get('mailName')
            fianlcontext = mailName+'$'+context
            news = models.Message.objects.create(
                Content = fianlcontext,
                user = models.User.objects.get(Id=userid)
                )
            news.save()
            resData['status'] = 1
...
def getallmail(req):
    if req.method == "GET":
        ...
        return render(req, 'admina/getallmail.html',{
            "messages": messages
            })
...
def project_recmanage(req, uid=None):
    ...
    if req.method == "GET":
        if uid:
            ...
            recruits = models.Recruit.objects.all()
            for recruit in recruits:
                if recruit.Times == 0:
                    recruit.Times = "未开始"
                elif recruit.Times == 1:
                    recruit.Times = "初期招募"
                elif recruit.Times == 2:
                    recruit.Times="二次招募"
                else:
                    recruit.Times="招募结束"
            return render(req, "admina/project_recmanage.html",{'Recruits':recruits})
    else:
        pass


@csrf_exempt
def delete_recruit(req):
    """
    删除招募
    :param req:
    :return:
    ...
    if req.method == 'POST':
        try:
            recruitId = req.POST['recruitId']
            recruit = models.Recruit.objects.filter(Id=recruitId)
            recruit.delete()
            data = 1    # 删除成功
        except Exception as e:
            print (e)
...
def modify_recruit(req):
    ...
    if req.method == 'POST':
        recruitId = req.POST['recruitId']
        times = req.POST['times']
        predictNumber = req.POST['predictNumber']
        recruitedNumber = req.POST['recruitedNumber']
        ...
        if times == "未开始":
            ...
            times = 3
        try:
            recruit = models.Recruit.objects.filter(Id=recruitId).update(Times=times,
    ...
    if req.method == "GET":
        ...
        if uid:
            ...
            try:
                ...
                if recruit_end_time:
                    recruit_end_time = recruit_end_time.split('-')
...
def creation_all(req, page=1, category=None):
    ...
    if req.method == "GET":
        itemsPerPage = 18
        pages = math.ceil(float(models.Creation.objects.filter(IsUse=True).count()) / float(itemsPerPage))# 计算页数
        print pages
        Labels = models.ProjectLabel.objects.all()
        Creations = models.Creation.objects.filter(IsUse=True)[(page-1)*itemsPerPage:page*itemsPerPage]
...
def comment_project(req):
    ...
    '''
    if req.method == "GET":
        Comments = models.Comment.objects.all().order_by('project')
...
def comment_user(req):
    ...
    '''
    if req.method == "GET":
        comments = models.Comment.objects.all().order_by("user")
        return render(req, 'admina/comment_realuser.html',{
                "comments":comments
            })
    else:
        pass
...
def comment_update(req):
    ...
    if req.method == "POST":
        ...
        try:
            comment = models.Comment.objects.get(Id=req.POST['commentid'])
            if req.POST['isuse'] == "1":
                comment.IsUse = True
            else:
                comment.IsUse = False
